...
DataFit2D::DataFit2D(const std::string& name, std::vector<TH2D*> htmps, TH2D* hsmp, TH2D* href, bool hist_build, const std::string& hist_xtitle, const std::string& hist_ytitle, const std::string& hist_ztitle) {
    ...
    for (auto&& htmp : htmps) {
        ...
        data_tmps_x_  .push_back(std::get<2>(htmp_data));
        data_tmps_y_  .push_back(std::get<3>(htmp_data));
    }
    ...
    data_smp_x_   = std::get<2>(hsmp_data);
    data_smp_y_   = std::get<3>(hsmp_data);
    ...
    if (href != nullptr) {
        ...
        data_ref_x_   = std::get<2>(href_data);
        data_ref_y_   = std::get<3>(href_data);
    }
...
std::tuple<double, std::vector<std::array<double, 2>>, std::vector<std::array<double, 2>>, std::vector<std::array<double, 2>>> DataFit2D::build_data(TH2D* hist) {
    std::tuple<double, std::vector<std::array<double, 2>>, std::vector<std::array<double, 2>>, std::vector<std::array<double, 2>>> datas;
    std::get<0>(datas) = 0.0;

    int ndimx = hist->GetXaxis()->GetNbins();
    int ndimy = hist->GetYaxis()->GetNbins();

    if (hist == nullptr) return datas;
    std::vector<std::array<double, 2>> data(ndimy * ndimx, {0.0, 0.0});

    double total = 0.0;
    for (int iby = 1; iby <= ndimy; ++iby) {
    for (int ibx = 1; ibx <= ndimx; ++ibx) {
        if (hist->GetBinContent(ibx, iby) <= 0.0) continue;
        int idx = (iby-1) * ndimx + (ibx-1);
    ...
    //double error = std::sqrt((total - 1.0) / total) / total;
    for (auto&& arr : data) {
        arr[0] /= total;
        arr[1] /= total;
    ...
    std::vector<std::array<double, 2>> data_x(ndimx, {0.0, 0.0});
    for (int it = 0; it < ndimx; ++it) {
        double sum_val = 0.0;
        double sum_err = 0.0;
        for (int jt = 0; jt < ndimy; ++jt) {
            int idx = jt * ndimx + it;
            sum_val += data.at(idx)[0];
            sum_err += (data.at(idx)[0] > 0.0) ? data.at(idx)[1] * data.at(idx)[1] : 0.0;
        }
        data_x.at(it)[0] = sum_val;
        data_x.at(it)[1] = (sum_val > 0.0) ? std::sqrt(sum_err) : error;
    }
    
    std::vector<std::array<double, 2>> data_y(ndimy, {0.0, 0.0});
    for (int jt = 0; jt < ndimy; ++jt) {
        double sum_val = 0.0;
        double sum_err = 0.0;
        for (int it = 0; it < ndimx; ++it) {
        ...
        data_y.at(jt)[0] = sum_val;
        data_y.at(jt)[1] = (sum_val > 0.0) ? std::sqrt(sum_err) : error;
    }

    std::get<0>(datas) = total;
    std::get<1>(datas) = data;
    std::get<2>(datas) = data_x;
    std::get<3>(datas) = data_y;
    return datas;
}
...
bool DataFit2D::fit() {
    ...
    ceres::CostFunction* cost_function = new VirtualDataFit2D(data_ndimx_, data_ndimy_, data_ntmp_, data_tmps_x_, data_tmps_y_, data_smp_x_, data_smp_y_);
