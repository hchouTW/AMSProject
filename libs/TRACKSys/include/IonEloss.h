...
class IonEloss {
    public :
        IonEloss(const std::array<long double, 4>& kpa, const std::array<long double, 6>& mpv, const std::array<long double, 6>& sgm, const std::array<long double, 6>& mode, long double fluc = Numc::ZERO<long double>) : isfluc_(false), kpa_(kpa), mpv_(mpv), sgm_(sgm), mode_(mode), fluc_(fluc) { isfluc_ = (Numc::Compare(fluc_) > 0); if (!isfluc_) fluc_ = Numc::ZERO<long double>; }
        ~IonEloss() {}
        
        std::array<long double, 2> minimizer(long double x, long double igmbta) const;
