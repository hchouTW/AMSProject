...
bool VirtualPhyTrFit::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    for (auto&& hit : hits_) {
        ...
        if (hitTRK != nullptr) {
            ...
            if (hasJacbGlb && hitTRK->sqx()) jb(hitTRK->seqIDqx(), parIDeta_) += hitTRK->divqx_eta() * jbGG(4, 4);
            if (hasJacbGlb && hitTRK->sqy()) jb(hitTRK->seqIDqy(), parIDeta_) += hitTRK->divqy_eta() * jbGG(4, 4);
        }
        ...
        if (hitTOF != nullptr) {
            ...
            if (hasJacbGlb && hitTOF->st()) jb(hitTOF->seqIDt(), parIDeta_) += hitTOF->divt_eta() * jbGG(4, 4);
            if (hasJacbGlb && hitTOF->sq()) jb(hitTOF->seqIDq(), parIDeta_) += hitTOF->divq_eta() * jbGG(4, 4);
            if (hasJacbGlb && hitTOF->st() && opt_tsft_) jb(hitTOF->seqIDt(), parIDtsft_) += hitTOF->divtsft(); // TOF time shift
        }
        ...
        if (hitRICH != nullptr) {
            ...
            if (hasJacbGlb && hitRICH->sib()) jb(hitRICH->seqIDib(), parIDeta_) += hitRICH->divib_eta() * jbGG(4, 4);
        }
        ...
        if (hitTRD != nullptr) {
            ...
            if (hasJacbGlb && hitTRD->sel()) jb(hitTRD->seqIDel(), parIDeta_) += hitTRD->divel_eta() * jbGG(4, 4);
        }
