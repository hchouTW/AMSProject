...
long double IonTrEloss::FuncMpv(long double ibta, const std::array<long double, 6>& par) {
    ...
    long double loggb = std::log(ibsqr - Numc::ONE<long double>);
 
    long double mpv = par[0] + par[1] * ibsqr - par[2] * loggb + par[3] * Numc::HALF * std::erfc(par[4] * loggb + par[5]);
...
std::array<long double, 3> IonTrEloss::minimizer(long double x, long double ibta, long double igb) const {
    ...
    long double mpv = get_mpv(ibsqr, loggb); 
    long double sgm = get_sgm(ibsqr, loggb); 
