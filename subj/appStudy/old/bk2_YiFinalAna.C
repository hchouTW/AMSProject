...
int main(int argc, const char ** argv) {
	...
	std::string tagdir = "/afs/cern.ch/work/h/hchou/BSUB_ANALYSIS/antipp5/NOW3__";
	...
	Int_t lBin[2] = {  1, 15 };
	Int_t iBin[2] = { 12, 38 };
	Int_t hBin[2] = { 28, 57 }; // L1 52(54), L9 52(54), Fs 57
	Int_t cbBin[4] = { 0, 13, 32, 57 };
	
	// Binning
	//Int_t lBin[2] = {  1,  6 };
	//Int_t iBin[2] = {  5, 12 };
	//Int_t hBin[2] = { 10, 17 };
	//Int_t cbBin[4] = { 0, 5, 10, 17 };
  ...
  //Axis AXir((TH1*)fAppBin->Get("hbin1"), Axis::kX);
	...
	for (Int_t ib = lBin[0]; ib <= lBin[1]; ++ib) {
		...
		if (ib > cbBin[0] && ib <= cbBin[1]) {
			...
			Apprlt_full->pushPointWithError(Graph::Point_t(cen, valOfApp), Graph::Error_t(0.0, errOfAppTotl));
		}
	}
	...
	for (Int_t ib = 1; ib <= AXnr.nbin(); ++ib) {
		...
		if (AXnr.bins(ib-1) >= 1.0 && AXnr.bins(ib) <= 800.) {
			...
			if (MgntNum::Valid(crrOfAppAccp)) LAppcrr_accp->pushPointWithError(Graph::Point_t(cen, crrOfAppAccp), Graph::Error_t(0.0,  crrOfAppAccpErr));
			Double_t crrOfAppAccpPred = 3.38751e-01 * TMath::Power(cen, -7.73513e-01) + 1.03987e+00;
			Double_t errOfAppAccp = crrOfAppAccp - crrOfAppAccpPred;
			if (MgntNum::Valid(errOfAppAccp)) LApperr_accp->pushPointWithError(Graph::Point_t(cen, errOfAppAccp), Graph::Error_t(0.0, crrOfAppAccpErr));
		}
	}
	...
	for (Int_t ib = 1; ib <= AXnr.nbin(); ++ib) {
		...
		if (AXnr.bins(ib-1) >= 1.0 && AXnr.bins(ib) <= 800.) {
			...
			Double_t crrOfAppAccp = ratACCPR / ratACCAP;
			Double_t crrOfAppAccpErr = crrOfAppAccp * std::sqrt(1./numOfSel_ACCPR+1./numOfSel_ACCAP);
			if (MgntNum::Valid(crrOfAppAccp)) IAppcrr_accp->pushPointWithError(Graph::Point_t(cen, crrOfAppAccp), Graph::Error_t(0.0,  crrOfAppAccpErr));
			Double_t crrOfAppAccpPred = 2.34627e-01 * TMath::Power(cen, -6.23656e-01) + 1.03143e+00;
			Double_t errOfAppAccp = crrOfAppAccp - crrOfAppAccpPred;
			if (MgntNum::Valid(errOfAppAccp)) IApperr_accp->pushPointWithError(Graph::Point_t(cen, errOfAppAccp), Graph::Error_t(0.0, crrOfAppAccpErr));
		}
	}
	...
	Hist * hH_C090 = Hist::New("hHp_C090_Evt", (TH1*)fPrC090->Get("hHp_Evt"));
	Hist * hH_C110 = Hist::New("hHn_C110_Evt", (TH1*)fPrC110->Get("hHp_Evt"));
	...
	for (Int_t ib = hBin[0]; ib <= hBin[1]; ++ib) {
		...
		Double_t numOfL1Pr     = (*hH_L1pos)()->Integral(AXcc.find(-3.), AXcc.find(2.0));
		Double_t errOfL1PrStat = std::sqrt(numOfL1Pr);
		Double_t numOfL1Smp    = (*hH_L1neg)()->Integral(AXcc.find(-3.), AXcc.find(2.0));
		if (numOfL1Smp > 20. && cen < 175.) {
			Fit::RooParam param("ccest", hH_L1neg, Fit::RooParam::VLIST({ hH_L1bkg, hH_L1sig }), -3., 2.0);
			Fit::RooSysResult rlt(param);
		...
		Double_t numOfL9Pr     = (*hH_L9pos)()->Integral(AXcc.find(-3.), AXcc.find(2.0));
		Double_t errOfL9PrStat = std::sqrt(numOfL9Pr);
		Double_t numOfL9Smp    = (*hH_L9neg)()->Integral(AXcc.find(-3.), AXcc.find(2.0));
		if (false && numOfL9Smp > 20. && cen < 175.) {
			Fit::RooParam param("ccest", hH_L9neg, Fit::RooParam::VLIST({ hH_L9bkg, hH_L9sig }), -3., 2.0);
			Fit::RooSysResult rlt(param);
			if (rlt.valid() && rlt.sysValid()) {
	...
	for (Int_t ib = 1; ib <= AXnr.nbin(); ++ib) {
		...
		if (AXnr.bins(ib-1) >= 1.0 && AXnr.bins(ib) <= 200.) {
			...
			Double_t errOfAppXsec = std::sqrt(2.0) * (ratC090 - ratC110) / (ratC110 + ratC090);
			HApperr_xsec->pushPoint(Graph::Point_t(cen, 100. * errOfAppXsec));
		}
		...
		if (AXnr.bins(ib-1) >= 1.0 && AXnr.bins(ib) <= 800.) {
			...
			Double_t crrOfAppAccp = ratACCPR / ratACCAP;
			Double_t crrOfAppAccpErr = crrOfAppAccp * std::sqrt(1./numOfSel_ACCPR+1./numOfSel_ACCAP);
			if (MgntNum::Valid(crrOfAppAccp)) HAppcrr_accp->pushPointWithError(Graph::Point_t(cen, crrOfAppAccp), Graph::Error_t(0.0,  crrOfAppAccpErr));
			Double_t crrOfAppAccpPred = 3.16258e-01 * TMath::Power(cen, -6.30666e-01) + 1.03140e+00;
			Double_t errOfAppAccp = crrOfAppAccp - crrOfAppAccpPred;
			if (MgntNum::Valid(errOfAppAccp)) HApperr_accp->pushPointWithError(Graph::Point_t(cen, errOfAppAccp), Graph::Error_t(0.0, crrOfAppAccpErr));
		}	
	}
	...
	editor.create();
	editor.cd(1, Canvas::AxisScl_t(1, 0));
	Apprlt_full->setStyle(Area(), Line(kRed), Marker(kRed));
	Graph::Collect("FApprlt_full", ";|Rigidity| [GV];#bar{p}/p Ratio", Graph::VLIST({ Apprlt_full }))->Draw("ap");
	//Apprlt_Pamela->Draw("same p");
	Apprlt_AMSPub->Draw("same p");
	editor.save();
