...
int main(int argc, char* argv[]) {
    ...
    PdfEditor editor(Window(), "fit_mutr_rh", "out/doc");

    TF1* func = new TF1("func", "gaus");

    const Axis& AXbta = Hist::Head("hRH_B_sqrm")->xaxis();
    ...
    for (int it = 1; it <= AXbta.nbin(); ++it) {
        double tme_hc = (*Hist::Head("hHCmutr_B_numt"))()->GetBinContent(it) / (*Hist::Head("hHCmutr_B_dent"))()->GetBinContent(it);
        
        (*hHCtme)()->SetBinContent(it, tme_hc);
        (*hHCtme)()->SetBinError  (it, 0.0);
    }
        
    std::vector<Hist*> vhOF_sqrm = Hist::ProjectAll(HistProj::kY, Hist::Head("hRH_B_sqrm"));
    std::vector<Hist*> vhHC_sqrm = Hist::ProjectAll(HistProj::kY, Hist::Head("hHCmutr_B_sqrm"));
    ...
    for (int it = 1; it <= AXbta.nbin(); ++it) {
        ...
        leg()->AddEntry((*vhHC_sqrm.at(it))(), "New", "lp");
        leg()->SetFillColor(0);
        leg.draw();
        editor.save();
    ...
    Legend leg_tme("", PadWindow(0.75, 0.85, 0.75, 0.80));
    ...
    combined_relmen->GetHistogram()->GetXaxis()->SetRangeUser(rh_rng[0], rh_rng[1]);
    ...
    TFile * ofle = new TFile("out/doc/fit_mutr_rh.root", "RECREATE");
    ofle->cd();
