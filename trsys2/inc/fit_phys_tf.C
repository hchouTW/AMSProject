...
int main(int argc, char* argv[]) {
    ...
    PdfEditor editor(Window(), "fit_phys_tf", "out/doc");

    TF1* func = new TF1("func", "gaus");

    const Axis& AXrig = Hist::Head("hHC_in_rig")->xaxis();

    std::array<double, 2> trpt_rng({ 0.6 * chrg, 20.0 * chrg });
    ...
    for (int it = 1; it <= AXrig.nbin(); ++it) {
        double tme_gm = (*Hist::Head("hHC_in_numt"))()->GetBinContent(it) / (*Hist::Head("hHC_in_dent"))()->GetBinContent(it);
        double tme_ph = (*Hist::Head("hHCphys_TQ_numt"))()->GetBinContent(it) / (*Hist::Head("hHCphys_TQ_dent"))()->GetBinContent(it);
    ...
    std::vector<Hist*> vhOF_rig = Hist::ProjectAll(HistProj::kY, Hist::Head("hCK_in_rig"));
    std::vector<Hist*> vhGM_rig = Hist::ProjectAll(HistProj::kY, Hist::Head("hHC_in_rig"));
    std::vector<Hist*> vhPH_rig = Hist::ProjectAll(HistProj::kY, Hist::Head("hHCphys_TQ_rig"));
    ...
    hOFeft->style(Line(kBlack),   Marker(kBlack,   MarkerStyle(MarkerShape::kCircle)));
    ...
    hOFmen->style(Line(kBlack),   Marker(kBlack,   MarkerStyle(MarkerShape::kCircle)));
    ...
    hOFrso->style(Line(kBlack),   Marker(kBlack,   MarkerStyle(MarkerShape::kCircle)));
    ...
    hOFrelrso->style(Line(kBlack),   Marker(kBlack,   MarkerStyle(MarkerShape::kCircle)));
    ...
    Legend leg_tme("<< Inner >>", PadWindow(0.60, 0.80, 0.35, 0.50));
    ...
    Legend leg_eft("<< Inner >>", PadWindow(0.60, 0.80, 0.55, 0.70));
    ...
    Legend leg_men("<< Inner >>", PadWindow(0.60, 0.80, 0.55, 0.70));
    ...
    Legend leg_rso("<< Inner >>", PadWindow(0.60, 0.80, 0.55, 0.70));
    ...
    Legend leg_relrso("<< Inner >>", PadWindow(0.60, 0.80, 0.55, 0.70));
    ...
    TFile * ofle = new TFile("out/doc/fit_phys_tf.root", "RECREATE");
    ofle->cd();
