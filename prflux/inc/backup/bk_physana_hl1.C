...
double accp_func(double arig) {
    ...
    //double crr = 1.05034e+00 + 2.02249e-01 * std::exp(-1.53080e-01 * arig) + 1.27439e+00 * std::exp(-2.95486e+00 * arig) + 1.27439e+00 * std::exp(-2.95486e+00 * arig);
    return crr;
}
...
int main(int argc, char* argv[]) {
    ...
    TH1D*  hmcapp = (TH1D*)fmcapp->Get("hHl1_cnt_MC_FLUX10");
    ...
    TH1D*  hmcapm = (TH1D*)fmcapm->Get("hHl1_cnt_MC_FLUX10");
    ...
    Hist* hcc_cc = Hist::New("hHNl1_mva_cc", (TH1*)TFile::Open(Form("/eos/ams/user/h/hchou/AMSData/subj/apflux/20Jan15/mcpr_l1o9flux%s/YiMdst.root", subv.c_str()))->Get("hHNl1_mva_MC_FLUX27"));

    Hist::Load("YiMdst.root", Form("/eos/ams/user/h/hchou/AMSData/subj/apflux/20Jan15/iss%s", subv.c_str()));

    PdfEditor editor(Window(), "apflux_hl1", "out");

    const Axis& AXrig = Hist::Head("hHPl1_mva2")->xaxis();
 
    std::vector<THStack*> vhstack;
    std::vector<Hist*> vhist;

    std::vector<Hist*> vhcc_cc = Hist::ProjectAll(HistProj::kY, hcc_cc);
    std::vector<Hist*> vhcc_pos = Hist::ProjectAll(HistProj::kY, Hist::Head("hHPl1_mva2"));
    std::vector<Hist*> vhcc_neg = Hist::ProjectAll(HistProj::kY, Hist::Head("hHNl1_mva2"));
    ...
    TFile * ofle = new TFile("out/apflux_hl1.root", "RECREATE");
    ofle->cd();
