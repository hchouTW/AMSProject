...
int main(int argc, char* argv[]) {
    ...
    Hist* hcc_cc = Hist::New("hHNfs_mva_cc", (TH1*)TFile::Open(Form("/eos/ams/user/h/hchou/AMSData/subj/apflux/20Jan15/mcpr_l1o9flux%s/YiMdst.root", subv.c_str()))->Get("hHNfs3_mva_MC"));
    ...
    const Axis& AXrig = Hist::Head("hHPfs3_mva")->xaxis();
 
    std::vector<THStack*> vhstack;
    std::vector<Hist*> vhist;

    std::vector<Hist*> vhcc_cc = Hist::ProjectAll(HistProj::kY, hcc_cc);
    std::vector<Hist*> vhcc_pos = Hist::ProjectAll(HistProj::kY, Hist::Head("hHPfs3_mva"));
    std::vector<Hist*> vhcc_neg = Hist::ProjectAll(HistProj::kY, Hist::Head("hHNfs3_mva"));
