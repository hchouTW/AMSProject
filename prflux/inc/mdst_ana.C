...
int main(int argc, char** argv) {
    ...
    for (auto&& file : sublist) chainFLX.Add(file.c_str());
	...
	chain_statement += Format("Totally Events (FLX) : %ld\n", chainFLX.GetEntries());
    ...
    if (chainFLX.GetEntries() == 0) std::cerr << Format("Don't have event (FLX)\n");
    ...
    LOG_IF(ERROR, chainFLX.GetEntries() == 0) << Format("Don't have event");
    ...
    Analyzer analyzer(&runlist, &chainFLX, &chainLTF, &chainLRH, &chainIIN, &chainIEX, &chainHEX, &chainHFS);
