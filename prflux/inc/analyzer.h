...
class Analyzer {
    ...
    public :
        Analyzer(TChain* list, TChain* chainFLX, TChain* chainLTF, TChain* chainLRH, TChain* chainIIN, TChain* chainIEX, TChain* chainHEX, TChain* chainHFS) { init(); set_branch(list, chainFLX, chainLTF, chainLRH, chainIIN, chainIEX, chainHEX, chainHFS); }
        ~Analyzer() { init(); }
    ...
    protected :
        inline void init();
        inline void set_branch(TChain* list, TChain* chainFLX, TChain* chainLTF, TChain* chainLRH, TChain* chainIIN, TChain* chainIEX, TChain* chainHEX, TChain* chainHFS);

        bool build_hist();
        bool process_data(TChain* mdst, bool (Analyzer::*process)());

        bool process_data_flx();
...
inline void Analyzer::set_branch(TChain* list, TChain* chainFLX, TChain* chainLTF, TChain* chainLRH, TChain* chainIIN, TChain* chainIEX, TChain* chainHEX, TChain* chainHFS) {
    if (list    != nullptr) runlist = list;
    if (runlist != nullptr) tree = runlist->CopyTree("");

    if (chainFLX != nullptr) varsFLX.set_tree(chainFLX);
