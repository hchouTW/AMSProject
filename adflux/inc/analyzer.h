...
class Analyzer {
    ...
    public :
        Analyzer(TChain* list, TChain* chainTF, TChain* chainRH) { init(); set_branch(list, chainTF, chainRH); }
        ~Analyzer() { init(); }
    ...
    protected :
        inline void init();
        inline void set_branch(TChain* list, TChain* chainTF, TChain* chainRH);

        bool build_hist();
        bool process_data(TChain* mdst, bool (Analyzer::*process)());

        bool process_data_tf();
        bool process_data_rh();
};
...
inline void Analyzer::set_branch(TChain* list, TChain* chainTF, TChain* chainRH) {
    if (list    != nullptr) runlist = list;
    if (runlist != nullptr) tree = runlist->CopyTree("");
    
    if (chainTF != nullptr) adtreeTF = chainTF->CloneTree(0);
    if (chainRH != nullptr) adtreeRH = chainRH->CloneTree(0);

    if (chainTF != nullptr) varsTF.set_tree(chainTF);
    if (chainRH != nullptr) varsRH.set_tree(chainRH);
...
void Analyzer::write() {
    ...
    file->cd();
    tree->Write();
    if (adtreeTF) adtreeTF->Write();
    if (adtreeRH) adtreeRH->Write();
    file->Write();
}
