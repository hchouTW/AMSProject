...
int main(int argc, char* argv[]) {
    ...
    TH1D*  hmcpr = (TH1D*)fmcpr->Get("hM_cnt_MC_FLUX27");
    ...
    TH1D*  hmcap = (TH1D*)fmcap->Get("hM_cnt_MC_FLUX27");
    ...
    TH1D*  hmcapp = (TH1D*)fmcapp->Get("hM_cnt_MC_FLUX27");
    ...
    TH1D*  hmcapm = (TH1D*)fmcapm->Get("hM_cnt_MC_FLUX27");
    ...
    PdfEditor editor(Window(), "apflux_m", "out");

    const Axis& AXrig = Hist::Head("hMP_sqrm")->xaxis();
    const Axis& AXrig2 = Hist::Head("hTMP_sqrm")->yaxis();
    ...
    std::vector<Hist*> vhTsqrm_pr2  = Hist::ProjectAll(HistProj::kY, Hist::Head("hTRMP_sqrm_pr"));
    std::vector<Hist*> vhTsqrm_el2  = Hist::ProjectAll(HistProj::kY, Hist::Head("hTRMN_sqrm_el"));
...
/*
    ...
    for (int it = 1; it <= AXtme.nbin(); ++it) {
        ...
        for (int ir = 4; ir <= 15; ++ir) {
            ...
            DataFit1D fit1D(Form("T%3dR%3d", it, ir), { (TH1D*)((*vhTsqrm_pr2.at(ir))()), (TH1D*)((*vhTsqrm_el2.at(ir))()) }, (TH1D*)((*vhRsqrm_neg.at(ir))()), (TH1D*)((*vhRsqrm_pos.at(ir))()), true);
    ...
    TFile * ofle = new TFile("out/apflux_m.root", "RECREATE");
    ofle->cd();
