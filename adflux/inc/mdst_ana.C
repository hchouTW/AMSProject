...
int main(int argc, char** argv) {
    ...
    TChain chainTF("varsTF");
    TChain chainRH("varsRH");
    for (auto&& file : sublist) runlist.Add(file.c_str());
    for (auto&& file : sublist) chainTF.Add(file.c_str());
    for (auto&& file : sublist) chainRH.Add(file.c_str());

	std::string chain_statement;
    chain_statement += Format("\nROOT files\n");
	chain_statement += Format("Totally Events (TF) : %ld\n", chainTF.GetEntries());
	chain_statement += Format("Totally Events (RH) : %ld\n", chainRH.GetEntries());
    chain_statement += Format("\n");

    std::cout << chain_statement;
    LOG(INFO) << chain_statement;

    if (chainTF.GetEntries() == 0) std::cerr << Format("Don't have event (TF)\n");
    if (chainRH.GetEntries() == 0) std::cerr << Format("Don't have event (RH)\n");
    LOG_IF(ERROR, chainTF.GetEntries() == 0) << Format("Don't have event");
    LOG_IF(ERROR, chainRH.GetEntries() == 0) << Format("Don't have event");
    ...
    Analyzer analyzer(&runlist, &chainTF, &chainRH);
