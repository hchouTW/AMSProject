...
int main(int argc, char* argv[]) {
    ...
    PdfEditor editor(Window(), "antip_hl1", "out");

    const Axis& AXrig = Hist::Head("hHPl1_lchiy_lrvar")->xaxis();
 
    std::vector<THStack*> vhstack;
    std::vector<Hist*> vhist;

    std::vector<Hist*> vhcc_cc = Hist::ProjectAll(HistProj::kZY, hcc_cc);
    std::vector<Hist*> vhcc_pos = Hist::ProjectAll(HistProj::kZY, Hist::Head("hHPl1_lchiy_lrvar"));
    std::vector<Hist*> vhcc_neg = Hist::ProjectAll(HistProj::kZY, Hist::Head("hHNl1_lchiy_lrvar"));
    ...
    TFile * ofle = new TFile("out/antip_hl1.root", "RECREATE");
    ofle->cd();
