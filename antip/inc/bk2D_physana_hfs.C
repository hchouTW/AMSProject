...
int main(int argc, char* argv[]) {
    ...
    UInt_t cntpr = 0;
    TFile* fmcpr = TFile::Open(Form("/eos/ams/user/h/hchou/AMSData/subj/antip/19Sep10/mcpr%s/YiMdst.root", subv.c_str()));
    ...
    UInt_t cntap = 0;
    TFile* fmcap = TFile::Open(Form("/eos/ams/user/h/hchou/AMSData/subj/antip/19Sep10/mcap%s/YiMdst.root", subv.c_str()));
    ...
    Hist* hcc_cc = Hist::New("hHNfs_lchiy_lrvar_cc", (TH1*)TFile::Open(Form("/eos/ams/user/h/hchou/AMSData/subj/antip/19Sep10/mcprflux%s/YiMdst.root", subv.c_str()))->Get("hHNfs_lchiy_lrvar_v2"));

    Hist::Load("YiMdst.root", Form("/eos/ams/user/h/hchou/AMSData/subj/antip/19Sep10/iss%s", subv.c_str()));

    PdfEditor editor(Window(), "antip_hfs", "out");

    const Axis& AXrig = Hist::Head("hHPfs_lchiy_lrvar_v2")->xaxis();
 
    std::vector<THStack*> vhstack;
    std::vector<Hist*> vhist;

    std::vector<Hist*> vhcc_cc = Hist::ProjectAll(HistProj::kZY, hcc_cc);
    std::vector<Hist*> vhcc_pos = Hist::ProjectAll(HistProj::kZY, Hist::Head("hHPfs_lchiy_lrvar_v2"));
    std::vector<Hist*> vhcc_neg = Hist::ProjectAll(HistProj::kZY, Hist::Head("hHNfs_lchiy_lrvar_v2"));
