...
double accp_func(double arig) {
    double crr = 1.04314e+00 + 1.37738e+00 * std::exp(-2.40246e+00 * arig) + 1.48223e-01 * std::exp(-1.26040e-01 * arig);
    return crr;
}
...
int main(int argc, char* argv[]) {
    ...
    TH1D*  hmcpr = (TH1D*)fmcpr->Get("hHl9_cnt_MC_FLUX10");
    ...
    TH1D*  hmcap = (TH1D*)fmcap->Get("hHl9_cnt_MC_FLUX10");
    ...
    //Hist* hcc_cc = Hist::New("hHNl9_lchiy_lrvar_cc", (TH1*)TFile::Open(Form("/eos/ams/user/h/hchou/AMSData/subj/antip/19Nov03/mcprflux%s/YiMdst.root", subv.c_str()))->Get("hHNl9_lchiy_lrvar_MC_FLUX27"));
    Hist* hcc_cc = Hist::New("hHNl9_lchiy_lrvar_cc", (TH1*)TFile::Open(Form("/eos/ams/user/h/hchou/AMSData/subj/antip/19Nov03/mcprL1o9flux_%s/YiMdst.root", subv.c_str()))->Get("hHNl9_lchiy_lrvar_MC_FLUX27"));

    Hist::Load("YiMdst.root", Form("/eos/ams/user/h/hchou/AMSData/subj/antip/19Nov03/iss%s", subv.c_str()));

    PdfEditor editor(Window(), "antip_hl9", "out");

    const Axis& AXrig = Hist::Head("hHPl9_lchiy_lrvar")->xaxis();
 
    std::vector<THStack*> vhstack;
    std::vector<Hist*> vhist;

    std::vector<Hist*> vhcc_cc = Hist::ProjectAll(HistProj::kZY, hcc_cc);
    std::vector<Hist*> vhcc_pos = Hist::ProjectAll(HistProj::kZY, Hist::Head("hHPl9_lchiy_lrvar"));
    std::vector<Hist*> vhcc_neg = Hist::ProjectAll(HistProj::kZY, Hist::Head("hHNl9_lchiy_lrvar"));
    ...
    for (int ir = 30; ir <= AXrig.nbin()-8; ++ir) {
    ...
    TFile * ofle = new TFile("out/antip_hl9.root", "RECREATE");
    ofle->cd();
