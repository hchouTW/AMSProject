#ifndef __CherenkovMeas_H__
#define __CherenkovMeas_H__
...
class CherenkovHit {
    ...
    public :
        ...
        // expert only
        inline void set_wgt(double wgt) { wgt_ = ((status_ && wgt > 0.0) ? wgt : 0.0); }
        inline void set_cnt(double cnt) { cnt_ = ((status_ && cnt > 0.0) ? cnt : 0.0); }
...
class CherenkovFit {
    public :
        ...
        CherenkovFit(const std::vector<CherenkovHit>& args_hits, const std::array<double, 2>& pmtc, double rfr_index, double width_bta, double bta_crr = 1.0);
    ...
    protected :
        ...
        inline bool is_within_pmtc(double cx, double cy) { return (std::hypot(cx - pmtc_[0], cy - pmtc_[1]) < (WIDTH_CELL * 6.0)); }
    ...
    private :
        static constexpr double INV_SQRT_TWO = 7.07106781186547462e-01;
...
class CherenkovRayTrace {
    ...
    private :
        ...
        static constexpr double TWO_PI = 6.28318530717958624e+00;

    public :
        CherenkovRayTrace() : border_(0), trace_(0), accuracy_(0) {}
        CherenkovRayTrace(const std::array<double, 6>& part, double cbta, double index, short kind, short tile);
        ~CherenkovRayTrace() {}
        
        void cal(const CherenkovCloud* cloud = nullptr);
  
        inline const double& border()   const { return border_; }
        inline const double& trace()    const { return trace_; }
        inline const double& accuracy() const { return accuracy_; }

        inline const std::vector<double> phis() const { return phis_; }

    protected :
        std::vector<std::array<double, 3>> photon_trace_;
        std::array<std::array<short,  TRACE_NSET>, TRACE_NPHI> photon_types_;
...
// Axis Org := Tau x Rho
class CherenkovRayTrace::OrthCoord {
    public :
