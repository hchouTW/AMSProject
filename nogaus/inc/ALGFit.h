class ALGFit {
    ...
    public :
        ALGFit(const std::vector<double>& hits, const TrSys::LandauGaus& alg);
...
class VirtualALGFit : public ceres::CostFunction {
    protected :
        const std::vector<double> hits_;
        const TrSys::LandauGaus   alg_;

        int nres_;
        int npar_;

    public :
        VirtualALGFit(const std::vector<double>& hits, const TrSys::LandauGaus& alg) : hits_(hits), alg_(alg), nres_(0), npar_(0) {
            nres_ = hits_.size();
            npar_ = 1;
...
ALGFit::ALGFit(const std::vector<double>& hits, const TrSys::LandauGaus& alg) : hits_(hits), alg_(alg) {
    // init params
    status_ = false;
    param_ = 0.0;
    error_ = 0.0;
    ...
    ceres::CostFunction* cost_function = new VirtualALGFit(hits_, alg_);
...
bool VirtualALGFit::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    int cnt_nhit = 0;
    for (auto&& hit : hits_) {
        std::array<double, 3>&& minimizer = alg_.minimizer(hit - param);
