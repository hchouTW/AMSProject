...
class HYC : public TObject {
	public :
		...
		void init() {
            ...
            std::fill_n(phys_status[0], 2*2, false);
            std::fill_n(phys_ndof_x[0], 2*2, 0);
            std::fill_n(phys_ndof_y[0], 2*2, 0);
            std::fill_n(phys_ndof_b[0], 2*2, 0);
            std::fill_n(phys_nchi_x[0], 2*2, 0);
            std::fill_n(phys_nchi_y[0], 2*2, 0);
            std::fill_n(phys_nchi_b[0], 2*2, 0);
            
            std::fill_n(phys_cen_loc[0][0], 2*2*3, 0);
            std::fill_n(phys_cen_dir[0][0], 2*2*3, 0);
            std::fill_n(phys_cen_rig[0], 2*2, 0);
            std::fill_n(phys_cen_bta[0], 2*2, 0);
            
            std::fill_n(phys_top_loc[0][0], 2*2*3, 0);
            std::fill_n(phys_top_dir[0][0], 2*2*3, 0);
            std::fill_n(phys_top_rig[0], 2*2, 0);
            std::fill_n(phys_top_bta[0], 2*2, 0);
            
            std::fill_n(phys_cpu_time[0], 2*2, 0);
            
            std::fill_n(mutr_status, 2, false);
            std::fill_n(mutr_ndof_x, 2, 0);
            std::fill_n(mutr_ndof_y, 2, 0);
            std::fill_n(mutr_ndof_b, 2, 0);
            std::fill_n(mutr_nchi_x, 2, 0);
            std::fill_n(mutr_nchi_y, 2, 0);
            std::fill_n(mutr_nchi_b, 2, 0);
            
            std::fill_n(mutr_mass, 2, 0);
            std::fill_n(mutr_sqrm, 2, 0);
            
            std::fill_n(mutr_cen_loc[0], 2*3, 0);
            std::fill_n(mutr_cen_dir[0], 2*3, 0);
            std::fill_n(mutr_cen_rig, 2, 0);
            std::fill_n(mutr_cen_bta, 2, 0);
            
            std::fill_n(mutr_top_loc[0], 2*3, 0);
            std::fill_n(mutr_top_dir[0], 2*3, 0);
            std::fill_n(mutr_top_rig, 2, 0);
            std::fill_n(mutr_top_bta, 2, 0);
            
            std::fill_n(mutr_cpu_time, 2, 0);
        }
    ...
    public :
        ...
        Bool_t  phys_status[2][2];
        Short_t phys_ndof_x[2][2];
        Short_t phys_ndof_y[2][2];
        Short_t phys_ndof_b[2][2];
        Float_t phys_nchi_x[2][2];
        Float_t phys_nchi_y[2][2];
        Float_t phys_nchi_b[2][2];

        Float_t phys_cen_loc[2][2][3];
        Float_t phys_cen_dir[2][2][3];
        Float_t phys_cen_rig[2][2];
        Float_t phys_cen_bta[2][2];

        Float_t phys_top_loc[2][2][3];
        Float_t phys_top_dir[2][2][3];
        Float_t phys_top_rig[2][2];
        Float_t phys_top_bta[2][2];
        
        Float_t phys_cpu_time[2][2];

        // Mass Fitting
        // inn-time-chrg, inn-cherenkov
        // state (lx ly lz ux uy uz rig beta)
        Bool_t  mutr_status[2];
        Short_t mutr_ndof_x[2];
        Short_t mutr_ndof_y[2];
        Short_t mutr_ndof_b[2];
        Float_t mutr_nchi_x[2];
        Float_t mutr_nchi_y[2];
        Float_t mutr_nchi_b[2];
        
        Float_t mutr_sqrm[2];
        Float_t mutr_mass[2];
        
        Float_t mutr_cen_loc[2][3];
        Float_t mutr_cen_dir[2][3];
        Float_t mutr_cen_rig[2];
        Float_t mutr_cen_bta[2];
        
        Float_t mutr_top_loc[2][3];
        Float_t mutr_top_dir[2][3];
        Float_t mutr_top_rig[2];
        Float_t mutr_top_bta[2];
        
        Float_t mutr_cpu_time[2];
