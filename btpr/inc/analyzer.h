...
class Analyzer {
    ...
    public :
        Analyzer(TChain* list, TChain* chainBT) { init(); set_branch(list, chainBT); }
        ~Analyzer() { init(); }
    ...
    protected :
        inline void init();
        inline void set_branch(TChain* list, TChain* chainBT);

        bool build_hist();
        bool process_data(TChain* mdst, bool (Analyzer::*process)());

        bool process_data_bt();
};
...
inline void Analyzer::set_branch(TChain* list, TChain* chainBT) {
    if (list    != nullptr) runlist = list;
    if (runlist != nullptr) tree = runlist->CopyTree("");

    if (chainBT != nullptr) varsBT.set_tree(chainBT);
}
