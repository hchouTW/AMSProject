...
int main(int argc, char** argv) {
    ...
    TChain chainBT("varsBT");
    for (auto&& file : sublist) runlist.Add(file.c_str());
    for (auto&& file : sublist) chainBT.Add(file.c_str());

	std::string chain_statement;
    chain_statement += Format("\nROOT files\n");
	chain_statement += Format("Totally Events (BT) : %ld\n", chainBT.GetEntries());
    chain_statement += Format("\n");

    std::cout << chain_statement;
    LOG(INFO) << chain_statement;

    if (chainBT.GetEntries() == 0) std::cerr << Format("Don't have event (BT)\n");
    LOG_IF(ERROR, chainBT.GetEntries() == 0) << Format("Don't have event");
    ...
    Analyzer analyzer(&runlist, &chainBT);
