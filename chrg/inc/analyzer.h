...
class Analyzer {
    ...
    public :
        Analyzer(TChain* list, TChain* chainDB) { init(); set_branch(list, chainDB); }
        ~Analyzer() { init(); }
    ...
    protected :
        inline void init();
        inline void set_branch(TChain* list, TChain* chainDB);

        bool build_hist();
        bool process_data(TChain* mdst, bool (Analyzer::*process)());

        bool process_data_db();
};
...
inline void Analyzer::set_branch(TChain* list, TChain* chainDB) {
    if (list    != nullptr) runlist = list;
    if (runlist != nullptr) tree = runlist->CopyTree("");
    
    if (chainDB != nullptr) treeSEL = chainDB->CloneTree(0);
    
    if (chainDB != nullptr) varsDB.set_tree(chainDB);
}
