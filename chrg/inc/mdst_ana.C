...
int main(int argc, char** argv) {
    ...
    for (auto&& file : sublist) chainDB.Add(file.c_str());

	std::string chain_statement;
    chain_statement += Format("\nROOT files\n");
	chain_statement += Format("Totally Events (DB) : %ld\n", chainDB.GetEntries());
    chain_statement += Format("\n");

    std::cout << chain_statement;
    LOG(INFO) << chain_statement;

    if (chainDB.GetEntries() == 0) std::cerr << Format("Don't have event (DB)\n");
    LOG_IF(ERROR, chainDB.GetEntries() == 0) << Format("Don't have event");
    ...
    Analyzer analyzer(&runlist, &chainDB);
